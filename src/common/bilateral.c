...
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(b,in,ox,oy,oz)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(buf,w1,w2,offset1,offset2,offset3,size1,size2,size3)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(buf,w0,w1,w2,offset1,offset2,offset3,size1,size2,size3)
#endif
  for(int k = 0; k < size1; k++)
  {
    size_t index = (size_t)k * offset1;
...
void dt_bilateral_slice(const dt_bilateral_t *const b, const float *const in, float *out, const float detail)
{
  ...
  const int oz = b->size_y * b->size_x;
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(norm,ox,oy,oz,b,in,out)
#endif
  for(int j = 0; j < b->height; j++)
  {
    size_t index = 4 * j * b->width;
